<template>
	<view>
		...
		<view class="bg-white d-flex a-center"
			...
			<!-- 中间 -->
			<input type="text" v-model="keyword" class="flex-1 bg-light rounded ml-3 px-2" style="height: 65rpx"
				placeholder="智能积木" />
			<!-- 右边 -->
	...
	export default {
		...
		methods: {
			...
			format(res) {
				return res.map(item => {
					var good_num =
						item.comments_count === 0 ? 0 : (item.comments_good_count / item.comments_count) * 100;
					return {
						id: item.id,
						title: item.title,
						titlepic: item.cover,
